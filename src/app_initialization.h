...
void initialize_photoshop(ChemEngine &moga) {
    ...
    v_VerticalLayout *vlayout = new v_VerticalLayout({{725, 150}, {250, 250}}, {{0.1, 0.1}, {0.9, 0.9}}, 5, nullptr, nullptr);
    vlayout->set_appearence(appr_frame_box);
    
    moga.add_view(vlayout);
    moga.resman.add(col_menu);
    moga.resman.add(appr_frame_box);
    ...
    auto b3d_idle = new AppearenceTexture(Resources.texture.button.b3d.idle);
    auto b3d_pressed = new AppearenceTexture(Resources.texture.button.b3d.pressed);
    moga.resman.add(colbutt);
    v_Button *butt = new v_Button({{0, 0}, {0, 0}}, (SmartColor*) colbutt);
    butt->e_mouse_press.add(new SpawnBallLambda(&moga));
    hlayout->layout_add(butt);

    // butt->set_appearence(b3d_idle);

    // butt->e_mouse_press.add(new AVAnimatorPress(butt, b3d_pressed));
    // butt->e_mouse_release.add(new AVAnimatorRelease(butt, nullptr, b3d_idle));
    
    // butt->set_view_id("aaa");
